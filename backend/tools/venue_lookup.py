...
from memory.vectordb import MIN_CACHE_HITS, save_venues, search_cached_venues
...
def _google_places_search(query: str, limit: int) -> list[dict]:
...
def lookup_venues(event: EventInput, limit: int = DEFAULT_LIMIT) -> list[dict]:
    ...
    cached = search_cached_venues(query, event.city, limit=limit)
    if len(cached) >= MIN_CACHE_HITS:
        return cached

    venues = _google_places_search(query, limit)
    if venues:
        save_venues(event.city, venues)
